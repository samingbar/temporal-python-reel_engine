...
# LLM Instance (Global)
# ==================================================
llm = ChatOpenAI(model_name=MODEL_NAME, temperature=TEMPERATURE, max_tokens=MAX_MODEL_TOKENS)
...
# ==================================================
def build_script_prompt(idea: str | dict[str, Any], dog_profile: str) -> tuple[list[Any], str]:
    ...
    user_message = SCRIPT_USER_TEMPLATE.format(
        dog_profile=dog_profile.strip(), idea_json=idea_json, schema_block=SCHEMA_BLOCK.strip()
    )
    return [
        SystemMessage(content=SCRIPT_SYSTEM),
        HumanMessage(content=user_message),
    ], f"{idea_ref}::{source_type}"
...
def generate_single_script(
    idea: str | dict[str, Any], dog_profile: str, repair_attempts: int = REPAIR_ATTEMPTS
) -> ScriptResult:
...
def generate_scripts_parallel(
    reel_ideas: Iterable[str | dict[str, Any]],
    dog_profile: str,
    max_workers: int = MAX_WORKERS,
    progress_callback: Callable[[ScriptResult], None] | None = None,
...
def compute_script_score(script: dict[str, Any]) -> tuple[float, dict[str, float], list[str]]:
    ...
    missing = REQUIRED_TOP_KEYS - set(script.keys())
    comps["structure_completeness"] = (
        1.0 if not missing else max(0, 1 - len(missing) / len(REQUIRED_TOP_KEYS))
    )
...
def generate_and_rank(
    ideas: Iterable[str | dict[str, Any]],
    dog_profile: str,
    save: bool = False,
...
if __name__ == "__main__":
    ...
    sample_ideas: list[str | dict[str, Any]] = [
        {
